...
export const deploy = async (subgraphType) => {
  ...
  const { node, deployKey } = getAlchemyDeploymentParams()

  try {
    console.log(`graph deploy --node ${node} --deploy-key ${deployKey} --version-label ${gitHashString} ${subgraphName} ${subgraphType}-subgraph.yaml`)
    const { stdout, stderr } = await exec(
      `graph deploy --node ${node} --deploy-key ${deployKey} --version-label ${gitHashString} ${subgraphName} ${subgraphType}-subgraph.yaml`
    )
